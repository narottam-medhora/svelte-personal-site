<script>
	...
	// Import data
	import data from '$lib/assets/data/olympics.json';
	...
	const colorScale = scaleOrdinal().domain(countries).range(colors).unknown('#666666');
...
>
	...
	{#if hovered}
		<div class="chart-tooltip" style="left: {position[0] + xNudge}px; top: {position[1]}px;">
			<p>{hovered.key}</p>
		</div>
	{/if}
</div>
...
<style>
	...
	path {
		transition: stroke-opacity 300ms ease, opacity 300ms ease;
	}
</style>
